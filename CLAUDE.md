...
goto-assistant is a self-hosted personal AI assistant with a web-based chat interface. It supports both Claude (Anthropic) and OpenAI as providers, with MCP (Model Context Protocol) server integration for extended capabilities like memory, scheduled tasks, and episodic recall. User-added MCP servers are routed through mcp-broker for dynamic tool discovery.
...
**Responses API vs Chat Completions** (OpenAI provider):
- The Responses API is only used when talking directly to OpenAI (`api.openai.com`), Azure OpenAI (`*.openai.azure.com`), or with no custom base URL. All other base URLs default to Chat Completions, which is universally supported by third-party proxies (LiteLLM, Ollama, vLLM, Groq, etc.) and translates correctly to non-OpenAI backends like Anthropic. `isResponsesAPICapable()` in `src/config.ts` controls this. When using Chat Completions: `setOpenAIAPI("chat_completions")` switches the SDK mode, and `shellTool()` is excluded (it's a hosted tool, Responses API only). The model listing endpoint also falls back from `/v1/models` to `/models` for gateways that don't use the `/v1` prefix.
...
**Persistence**: `src/sessions.ts` — SQLite (better-sqlite3, WAL mode) stores conversation metadata and messages in `data/sessions.db`. Claude uses `sdk_session_id` for resumption; OpenAI replays history from the messages table. Setup chat conversations are stored with `mode = 1` and filtered from `listConversations()` so they don't appear in the sidebar.
...
**Configuration**: `src/config.ts` — app config stored in `data/config.json`, MCP server config stored separately in `data/mcp.json`. `mcp.json` is the single source of truth for MCP servers — edited directly by users, read by the app. Two derived files are auto-generated by `syncBrokerConfig()`: `data/mcp-broker/servers.json` (user-added servers for the broker to manage) and `data/mcp-agent.json` (built-in + broker servers for agents and mcp-cron). `BUILTIN_SERVER_NAMES` defines the set of app-managed servers (cron, memory, time, messaging, episodic-memory, broker) — everything else is considered user-added and routed through the broker. Agents receive filtered servers via `getAgentMcpServers()`, which returns only built-in + broker when the broker is present, or all servers as fallback. Environment variables (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`) override file config. API keys are masked in API responses. `PORT` env var overrides configured port. `GOTO_DATA_DIR` env var overrides the data directory (default `./data` for dev, `~/.goto-assistant` via npx).
...
**Task dashboard** (`public/task-chat.js`, task sections in `index.html`): Sidebar "Tasks" tab lists mcp-cron tasks; clicking one opens a detail view with metadata, prompt/command, results, and an inline AI chat panel. Tasks can be run, enabled/disabled, and deleted via action buttons. `runTask()` uses fire-and-forget polling with a pending result queue: when a run completes while the user is viewing a different task, the result is queued in `taskRunState[taskId].pendingResult` and delivered when the user navigates back. There is only one shared `runBtn` DOM element — `renderTaskDetail()` always resets it to the correct state (idle/spinning) for the currently viewed task, so the pending path intentionally does not touch the button. Task API endpoints in `server.ts` proxy to mcp-cron via `callCronTool()` (defined in `src/cron.ts`); `GET /api/tasks` returns `[]` when cron is not running. When the user saves setup config (`POST /api/setup`) or the AI chat modifies MCP config (`POST /api/reload`), `reloadServices()` calls `syncBrokerConfig()` (syncs broker entry, user servers, agent config, and cron's `--mcp-config-path`) then `restartCronServer()` which fingerprints the cron config + `mcp-agent.json` content and skips restart if unchanged, otherwise stops the old mcp-cron instance and starts a fresh one with updated args (e.g. new provider/model, new broker filtering). `killProc()` sends SIGTERM to the npx process; the broken stdin pipe causes mcp-cron to exit on EOF. `stopCronServer()` awaits the child exit (5s timeout fallback) so `process.exit(0)` doesn't fire while mcp-cron is still alive. A synchronous `stopCronSync()` is called from `process.on("exit")` as a safety net for non-graceful shutdown paths.
**WhatsApp integration** (`src/whatsapp.ts`): Optional WhatsApp client using [Baileys](https://github.com/WhiskeySockets/Baileys) (WhatsApp Web multi-device protocol). Runs in-process — no Docker or external services needed. Auth state persisted in `data/whatsapp-auth/` via Baileys' `useMultiFileAuthState`, so QR scan is only needed once. Off by default (`whatsapp.enabled = false` in config); enabled via setup page toggle or Q&A wizard. **Self-chat only** — the agent only responds to messages in the user's own self-chat (WhatsApp's "Message yourself"), never to other people messaging the user's number. This prevents unintended AI replies to contacts. Self-chat detection uses `checkSelfChat()` which matches `remoteJid` against both the phone-number JID (`sock.user.id`, `@s.whatsapp.net`) and the LID (`sock.user.lid`, `@lid`) — WhatsApp can use either format for self-chat, and the format may change server-side without warning. Debug logging (`goto-assistant:whatsapp`) traces message filtering decisions. Loop prevention uses a `sentMessageIds` set to skip agent-sent messages. Messages are processed through the same `routeMessage()` pipeline as web chat. Key design: per-chatId queue (`Map<string, Promise>` chain) serializes message processing to avoid Claude session resumption conflicts; responses are buffered (not streamed) and split at ~65K chars for WhatsApp's message limit. Conversations are mapped via `channel_id` column on the conversations table (NULL for web, WhatsApp remoteJid for WA). API endpoints: `GET /api/whatsapp/status`, `GET /api/whatsapp/qr`, `POST /api/whatsapp/connect`, `POST /api/whatsapp/disconnect`. `sendWhatsAppMessage()` is an exported function for programmatic sends — takes text and optional `to` (phone number or `"self"`), normalizes to JID, splits at ~65K, tracks sent IDs for self-chat loop prevention. **Media support**: `sendWhatsAppMessage` accepts an optional `SendMediaOptions` with a `media` field (local file path or URL). Media is classified by MIME type (`lookupMimeType` → `classifyMediaType`) and sent as image/video/audio/document via Baileys. **GIF limitation**: Baileys' image pipeline uses sharp `extractImageThumb` which corrupts animated GIFs (multi-frame → broken upload). GIFs are sent as documents instead to preserve the original file. Proper animated GIF support would require ffmpeg conversion to mp4 with `gifPlayback: true`. Incoming GIFs from WhatsApp arrive as `videoMessage` with `gifPlayback` and are handled alongside images. **Upload references**: the `media` parameter supports `upload:{fileId}` syntax (resolved server-side in `POST /api/messaging/send` to the actual file path in `data/uploads/`). OpenAI messages with attachments include `[Attached file: upload:{fileId}]` text hints so the model can reference them.

**Messaging MCP server** (`src/messaging.ts`, `src/mcp-messaging.ts`): Generic messaging tool exposed via a built-in MCP server. Architecture: the main Express process owns all channel connections (singletons); a stateless MCP stdio server (`mcp-messaging`) proxies `send_message` tool calls to `POST /api/messaging/send`, which routes to the appropriate channel's send function via a channel registry (`src/messaging.ts`). Currently supports WhatsApp; designed to expand to Telegram, Signal, etc. by adding a connection module and calling `registerChannel()` — no MCP server changes needed. Auto-configured in `mcp.json` when any messaging channel is enabled (`syncMessagingMcpServer()` in `src/config.ts`). Design principle: simplicity over gateway complexity — no separate long-running gateway process, no extra ports, no WebSocket RPC. Both consumers get the tool automatically: mcp-cron reads `mcp-agent.json` via `--mcp-config-path` (auto-updated by `syncBrokerConfig()`), and the main agent loads servers from `getAgentMcpServers()`.

**Episodic memory MCP server** (`src/episodic.ts`, `src/mcp-episodic.ts`): Built-in MCP server providing full-text search over past conversations (`data/sessions.db`) and mcp-cron task results (`~/.mcp-cron/results.db`) using SQLite FTS5. Exposes 3 tools: `search_episodes` (full-text search with time-range/source filters and snippet highlighting), `get_conversation_context` (retrieve full message thread from a conversation), `list_recent_episodes` (browse recent activity chronologically). Task result querying is handled by mcp-cron's `query_task_result` tool instead. Uses watermark-based incremental indexing — `ensureIndex()` tracks the last indexed message/result ID per source in a separate `data/episodic-index.db`, only indexing new rows on each call. Source DBs are opened read-only and closed immediately after use. Query sanitization strips FTS5 special characters and converts words to prefix searches (`"word"*`). Auto-configured in `mcp.json` unconditionally via `syncEpisodicMcpServer()` in `src/config.ts` (called on startup, config reload, and migration v3). The MCP server receives `GOTO_DATA_DIR` and `MCP_CRON_DB_PATH` env vars; the cron DB path is resolved from the `"cron"` server's `--db-path` arg in `mcp.json`, falling back to `~/.mcp-cron/results.db`. Entry point: `bin/goto-assistant-episodic.js`. Graceful degradation: if either source DB doesn't exist, indexing skips that source and searches return empty results.

**MCP broker** (`src/config.ts` — `syncBrokerConfig()`): Always-on MCP gateway using [mcp-broker](https://github.com/jolks/mcp-broker) that exposes 7 meta-tools (`search_tools`, `call_tools`, etc.) instead of all individual MCP tools, reducing token waste and improving tool selection accuracy. User-added servers are routed through the broker's FTS5 full-text search for dynamic tool discovery; built-in servers (`BUILTIN_SERVER_NAMES`: cron, memory, messaging, episodic-memory, broker) connect directly. `syncBrokerConfig(servers?)` is the single facade that: (1) ensures the broker entry exists in `mcp.json`, (2) writes user-added servers to `data/mcp-broker/servers.json` (permissions `0o600`), (3) updates cron's `--mcp-config-path` to point to `mcp-agent.json`, and (4) writes the agent-facing config to `data/mcp-agent.json`. Called from `reloadServices()`, `POST /api/mcp-servers`, and startup. The broker process is spawned by the SDK as a stdio MCP server on each agent interaction — it reads the latest `servers.json` on startup, syncs its FTS5 index, and harvests tools from new/changed servers.
...
- MCP servers live in `data/mcp.json` (not `config.json`) and are passed as a separate parameter to agent functions. When the broker is present, agents receive only built-in + broker servers via `getAgentMcpServers()`; user-added servers are accessed through the broker's `search_tools`/`call_tools` meta-tools
